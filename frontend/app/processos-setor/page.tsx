...
export default function ProcessosSetorPage() {
  ...
  const loadProcessos = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/api/processos-setor', {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-8">
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-gray-800 mb-2">Processos do Setor</h1>
            <p className="text-gray-600">Processos tramitados para o seu setor</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl shadow-xl p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-700 mb-2">
                {filtro ? 'Nenhum processo encontrado' : 'Nenhum processo no seu setor'}
              </h3>
              <p className="text-gray-500">
                {filtro ? 'Tente ajustar os filtros de busca' : 'Quando processos forem tramitados para o seu setor, eles aparecerão aqui'}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-6">
              {processosFiltrados.map((processo) => (
                <div
                  key={processo.id}
                  className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all p-6 cursor-pointer border-l-4 border-green-500"
